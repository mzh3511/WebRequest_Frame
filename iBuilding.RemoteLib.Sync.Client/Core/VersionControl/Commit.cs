...
using RanOpt.Common.RemoteLib.Sync.Client.Core.VersionControl;
    ...
    public class Commit
    {
        ...
        public void Add(Commit commit)
        {
            ...
            foreach (var file in commit.Files)
            {
                ...
                if (files.Any(x => x.FileName == file.FileName))
                {
                    ...
                    if (files[fidx].DiffType == Filemap.FileDiff.Type.Created && file.DiffType != Filemap.FileDiff.Type.Deleted)
                        continue;
        ...
        public void Apply(string projectDir, string dataFile, bool deflate)
        {
            ...
            foreach (var file in Files)
            {
                ...
                if (file.DiffType == Filemap.FileDiff.Type.Deleted)
                {
            ...
            {
#if !SERVER
                if (file.DiffType != Filemap.FileDiff.Type.Deleted) // apply only for existing files
                {
                    var fileName = projectDir + "/" + file.FileName;
